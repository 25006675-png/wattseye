...
AC card (uses dedicated sensor, no AI uncertainty shown):

```text
Air Conditioner
Status: ON
Power: 1500W (Measured · Dedicated CT clamp)
...
The prototype ships **four tabs** (Dashboard / Coach / Bill / History). The earlier
Routine tab has been removed — routine evidence (heatmap, per-appliance baselines,
K-Means phases) now lives inline inside each Coach card under "Why this appeared,"
so the user sees routine data at the moment of decision rather than as a separate
descriptive surface.
...
### Page 4 — Coach (recommendations)

Shows up to 12 recommendation cards generated by `ML/insights/coach/`:

- Top 2 cards surfaced as "Top recommendations now" (highest ranker score)
- Remaining 10 shown as "More insights" below
- Each card has: family + severity tags, headline, impact sentence, action box (saving + effort + confidence), three buttons (Do this / Remind me / Not useful)
- Two expandables per card: **Why this appeared** (routine + ML evidence chain) and **How we calculated this** (TNB RP4 math, auditable)

Cards span 5 families: Waste, Tariff (TNB ToU + RP4 cliff), Forecast, Context (weather + anomaly), Capital (appliance upgrade with ST registry link).
